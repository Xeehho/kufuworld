# -*- coding: utf-8 -*-
"""视觉审计截图 runner: temp-inject ProbeShots -> run game windowed -> restore project.godot."""
...
gp = os.path.join(proj, "tools", "probe_shots_stdout.txt")
with open(pg, encoding="utf-8") as f:
    original = f.read()
marker = 'ProbeShots="*res://tools/probe_shots.gd"'
try:
...
shots = [f for f in os.listdir(os.path.join(proj, "tools")) if f.startswith("vshot_")]
