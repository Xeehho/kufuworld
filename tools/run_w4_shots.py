# -*- coding: utf-8 -*-
"""W4 目检截图 runner: temp-inject ProbeW4Shots -> run game -> restore project.godot."""
...
gp = os.path.join(proj, "tools", "probe_w4_shots_stdout.txt")
marker = 'ProbeW4Shots="*res://tools/probe_w4_shots.gd"'
...
log = os.path.join(proj, "tools", "probe_w4_shots_log.txt")
print(open(log, encoding="utf-8", errors="replace").read() if os.path.exists(log) else "NO LOG")
shots = sorted(f for f in os.listdir(os.path.join(proj, "tools")) if f.startswith("vshot_w4_"))
print("shots:", shots)
