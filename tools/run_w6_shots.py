# -*- coding: utf-8 -*-
"""W6 目检截图 runner: temp-inject ProbeW6Shots -> run game -> restore project.godot."""
...
gp = os.path.join(proj, "tools", "probe_w6_shots_stdout.txt")
marker = 'ProbeW6Shots="*res://tools/probe_w6_shots.gd"'
...
log = os.path.join(proj, "tools", "probe_w6_shots_log.txt")
print(open(log, encoding="utf-8", errors="replace").read() if os.path.exists(log) else "NO LOG")
shots = sorted(f for f in os.listdir(os.path.join(proj, "tools")) if f.startswith("vshot_w6_"))
print("shots:", shots)
