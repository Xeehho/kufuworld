# -*- coding: utf-8 -*-
"""样板区 P0 验收截图 runner: temp-inject ProbeDemoShots -> run game -> restore project.godot."""
...
gp = os.path.join(proj, "tools", "probe_demo_shots_stdout.txt")
marker = 'ProbeDemoShots="*res://tools/probe_demo_shots.gd"'
...
log = os.path.join(proj, "tools", "probe_demo_shots_log.txt")
print(open(log, encoding="utf-8", errors="replace").read() if os.path.exists(log) else "NO LOG")
shots = sorted(f for f in os.listdir(os.path.join(proj, "docs", "shots")) if f.startswith("demo_"))
print("shots:", shots)
